...
def read_data(
    ...
    if as_graphs:
        ...
        with open(graph_classes_path, 'r') as f:
            ...
            if len(empty_indices) > 0:
                print(f'Keys: {",".join([str(empty_index) for empty_index in empty_indices])} are empty.')
        #for i in range(1, len(Graphs)+1):
        #    Gs.append([Graphs[i], node_labels[i], edge_labels[i]])
    ...
    if with_classes:
        ...
        classes = [each_class for each_class, G in zip(classes, Gs) if G is not None and len(G[0]) > 0]
        Gs = [G for G in Gs if G is not None and len(G[0]) > 0]
